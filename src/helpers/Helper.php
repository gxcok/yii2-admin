...
class Helper
{
        ...
        if ($where && $params) {
            ...
            foreach ($params as $key => $value) {
                ...
                if (is_array($handle)) {
                    ...
                    if($key=='start_date'||$key=='end_date'||$key=='start_time'||$key=='end_time'){
                        $field      = ArrayHelper::getValue($handle, 'field');       // 对应字段
                        $expression      = ArrayHelper::getValue($handle, 'expression');       // 对应比较符
                    }else{
                ...
                //数据库表字段比较 ->where(['id' => new \yii\db\Expression('a_id')]) // 比较 id 和 a_id 字段
                if(strpos($value,"Expression|")!==false){
                    $val=explode("|",$value);//Expression|a_id
                    $arrReturn[] = [$key=>new \yii\db\Expression($val[1])];
                }else{
     ...
     */
    public static function getAssetUrl($assetClass = 'gxcok\admin\web\AdminAsset')
    {
