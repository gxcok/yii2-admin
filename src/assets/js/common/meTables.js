/**
 * Created by guorui on 2017/3/14.
 */
function sub_string(str, len,extDot="..."){
    if(str.length<=len){
        return str;
    }else{
        return str.substring(0, len)+extDot;
    }
}
(function (window, $) {
    // 获取数组信息
    ...
    meTables.inputCreate = function (params) {
        ...
        if (params.default) params.value = params.default;
    ...
    meTables.selectCreate = function (params, d, max_len=16) {
        html = "";
        if (d && this.isObject(d)) {
            var c = params.default;
            ...
            if(typeof(params.sort) == "undefined"){
                for (i in d) {
                    html += '<option value="' + i + '" ' + (i == c ? ' selected="selected" ' : "") + ' title="' + d[i] + '">' + sub_string(d[i],max_len) + '</option>';
                }
            }else{
                if(!params.sort){//当不重排选择项数组时
                    for (i in d) {
                        html += '<option value="' + d[i]['value'] + '" ' + (i == c ? ' selected="selected" ' : "") + ' title="' + d[i]['title'] + '">' + sub_string(d[i]['title'],max_len) + '</option>';
                    }
                }
            }
    ...
    meTables.selectInput = function (params, value, defaultObject, max_len=16) {
        html = "";
        if (defaultObject) {
            for (i in defaultObject) {
                html += '<option value="' + i + '" selected="selected" title="' + defaultObject[i] + '">' + sub_string(defaultObject[i],max_len) + '</option>';
            }
        }
        ...
        if (value) {
            if(typeof(params.sort) == "undefined"){
                for (i in value) {
                    html += '<option value="' + i + '" title="' + value[i] + '">' + sub_string(value[i],max_len) + '</option>';
                }
            }else{
                if(!params.sort){//当不重排选择项数组时
                    for (i in value) {
                        html += '<option value="' + value[i]['value'] + '" title="' + value[i]['title'] + '">' + sub_string(value[i]['title'],max_len) + '</option>';
                    }
                }
            }
        }
